...
int f4_read(struct f4_data *t) {
    return (scanf("%d.%d", &t->filed_A, &t->filed_B) == 2);
}
...
struct f4_data f4_diff(struct f4_data par1, struct f4_data par2) {
    par1.filed_B = (par2.filed_A-par1.filed_A)*1000+(par2.filed_B-par1.filed_B);
    par1.filed_A = par1.filed_B / 1000;
    par1.filed_B %= 1000;
    return par1;
}
...
#if !defined(COMPILEONBIRO) || defined(COMPILEFOR1)
  int eredmeny, i;
  struct {char str[128]; int eredmeny;} testlist[1] = {{"720739", 3}};
  for (i = 0; i < 1; ++i) {
