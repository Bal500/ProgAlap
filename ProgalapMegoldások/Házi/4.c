...
véletlenlenszeru helyekre beszurtak a szoszam hosszat jelzo szamokat.
...
void titkosit(char bemenet[], char kimenet[]) {
    ...
    while (bemenet[i] != '\0') {
        ciklus++;
        if (bemenet[i] == ' ') {
            kimenet[szoszam] = nulla;
            nulla = '0';
            hossz[szoszam + 1] = ciklus;
            szoszam++;

            int j = i;
            while (bemenet[j] != '\0') {
                bemenet[j] = bemenet[j + 1];
                j++;
            }
        } else {
            if (bemenet[i + 1] == '\0') {
                kimenet[szoszam] = nulla + 1;
                hossz[szoszam + 1] = ciklus + 1;
                szoszam++;
            }
        }
        nulla += 1;
    ...
    int ciklusford = 0;
    for (int l = 0; l < szoszam; l++) {
        for (int k = hossz[l + 1]; k >= hossz[l] + 1; k--) {
            kimenet[szoszam + ciklusford] = bemenet[k];
            ciklusford++;
        }
    }

    kimenet[szoszam + ciklusford] = '\0';
}
